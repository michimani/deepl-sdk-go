...
func TestTranslateTextBody(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expect: "auth_key=test-authn-key&source_lang=EN&split_sentences=0&target_lang=JA&text=text1&text=text2",
		},
		{
			name: "normal: all",
			params: params.TranslateTextParams{
			...
			expect: "auth_key=test+key&source_lang=EN&split_sentences=1&target_lang=JA&text=text+1&text=text+2",
		},
		{
			name:   "normal: empty",
			params: params.TranslateTextParams{},
